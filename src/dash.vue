    ...
    <v-main>
      ...
      <div v-if="!ready && delayed">
        <v-container
          ...
          <div class="text-subtitle-1 text-center" v-for="(err, ix) in paletteErrorsHtml" :key="ix">
            {{ err }}
          </div>
...
.iframe-tab-wrap.iframe-a--active {
  display: block;
}
...
.compact-input .v-field__field {
  padding-top: 0px !important;
}
.large-font .v-field__field {
...
export default {
  ...
  data: () => ({
    ...
    delayed: false, // turns true some hundreds milliseconds after mount
  }),
  ...
  computed: {
    ...
    gotPalette() {
      return this.palette.loaded && this.palette.errors.length == 0
    },
    ready() {
      return this.gotConfig && this.gotPalette
    },
    ...
    dash() {
      return this.ready ? this.$config.dash : {}
    },
    dash_tabs() {
      // tabs to show, handling init and handle deleted tabs
      return (this.dash.tabs || []).filter(
        id => id.startsWith("t") && id in this.$config.tabs && this.$config.tabs[id].id == id
      )
    },
    tab() {
      return this.tab_id ? this.tabs[this.tab_id] : {}
    },
    tab_id() {
      return this.tab_ix != null ? this.dash_tabs[this.tab_ix] : ""
    }, // current tab ID
    tabs() {
      // make easily accessible in template
    ...
    tabs_grids() {
      return Object.fromEntries(
        this.dash_tabs.map(t => [
          t,
          (this.tabs[t].grids || []).filter(id => id in this.grids && this.grids[id].id == id),
        ])
      )
    },
    canUndo() {
      return this.$store.undo.buf.length > 0
    },
    title() {
      return window.flexdash_options.title
    },

    iframe_a_show() {
      return this.tab.url && this.tab.slot == "a"
    },
    iframe_a_class() {
      return ["iframe-tab-wrap", { "iframe-a--active": this.iframe_a_show }]
    },
    iframe_b_show() {
      return this.tab.url && this.tab.slot == "b"
    },
    iframe_b_class() {
      return ["iframe-tab-wrap", { "iframe-b--active": this.iframe_b_show }]
    },
    tab_show() {
      return !!this.tab.grids
    },
    tabs_items_class() {
      return { "tabs--active": this.tab_show }
    },

    // palette.errorsHtml deals with the fact that each error is multi-line
    paletteErrorsHtml() {
      return this.palette.errors.join("\n").split("\n")
    },
  },
  ...
  watch: {
    ...
    dash_tabs(dt) {
      ...
      this.implementURL()
      if (this.tab_ix >= dt.length) {
        this.changeTab(dt.length - 1)
        console.log("Forcing tab_ix to", this.tab_ix)
      }
      if (this.tab_ix == null) {
        this.changeTab(0)
        console.log("Init tab_ix to 0")
      }
    },
  ...
  mounted() {
    ...
    setTimeout(() => (this.delayed = true), 500)
  },
  ...
  methods: {
    ...
    implementURL() {
      ...
      console.log(
        `Implementing URL: '${hash_route}' tab=${rTab} grid=${rGrid} #tabs=${this.dash_tabs.length}`
      )
      // figure out which tab we're supposed to be on
      if (rTab && rTab.startsWith("#")) {
        const ix = this.matchcontainer(
          rTab.substring(1),
          this.dash_tabs.map(id => this.tabs[id])
        )
        if (ix == null) return
        ...
        if (rGrid) {
          ...
          console.log(
            `tab_ix=${this.tab_ix} tab_id=${tab_id} grids=${grids.map(g => g.id).join(",")}`
          )
