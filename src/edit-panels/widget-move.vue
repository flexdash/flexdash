...
<template>
  <v-menu offset-y v-model="show">
    ...
    <template v-slot:activator="{ on:menu, props }">
      <v-tooltip bottom>
        <template v-slot:activator="{ on:tt }">
          <v-btn small icon :class="button_class" v-bind="props" v-on="{...tt, ...menu}">
    ...
    <v-list>
      ...
      <v-list-item v-for="g in grid_list" :key="g.id" @click="select(g.id)" link>
      ...
      <v-list-item v-for="p in panel_list" :key="p.id" @click="select(p.id)" link>
...
export default {
  ...
  data() { 
    ...
    if (!d.grid_id) throw new Error(`WidgetMove: widget ${this.widget_id} not found in any grid`)
