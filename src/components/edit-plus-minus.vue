...
export default {
  ...
  methods: {
    inc: function() { if (this.ix < this.range.length-1) this.ix += 1; this.emit(); },
    dec: function() { if (this.ix > 0) this.ix -= 1; this.emit(); },
    fmt: function() {
      let v = this.range[this.ix];
      if (typeof v == 'number') return "" + v + " " + this.unit;
      else return v;
    },
    emit: function() { this.$emit('update:value', this.range[this.ix]); },
  },
}
</script>
